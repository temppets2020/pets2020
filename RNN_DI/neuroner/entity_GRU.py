...
def bidirectional_GRU(inputs, hidden_state_dimension, initializer,
    ...
    with tf.variable_scope("bidirectional_GRU"):
        ...
        for direction in ["forward", "backward"]:
            with tf.variable_scope(direction):
                lstm_cell[direction] =  tf.contrib.rnn.GRUCell(hidden_state_dimension, kernel_initializer=initializer)
                # LSTM cell
                GRU_cell[direction] = tf.contrib.rnn.GRUCell(hidden_state_dimension, kernel_initializer=initializer)
        ...
        # outputs_gru, _ = tf.nn.bidirectional_dynamic_rnn(GRU_cell["forward"],
        #                                                  GRU_cell["backward"], inputs=inputs,
        #                                                  dtype=tf.float32,
        #                                                  sequence_length=sequence_length,
        #                                                  )


        # outputs = tf.contrib.rnn.MultiRNNCell([outputs_gru, outputs_lstm], state_is_tuple=True)

        if output_sequence == True:
            # tmp_forward = tf.contrib.rnn.MultiRNNCell([GRU_cell["forward"], lstm_cell["forward"]], state_is_tuple=True)
            # tmp_backward = tf.contrib.rnn.MultiRNNCell([GRU_cell["backward"], lstm_cell["backward"]],
            #                                            state_is_tuple=True)
            outputs_lstm, final_states = tf.nn.bidirectional_dynamic_rnn(GRU_cell["forward"], GRU_cell["backward"], inputs=inputs,
                                                                         dtype=tf.float32,
                                                                         sequence_length=sequence_length)
                                                                         # initial_state_fw=initial_state["forward"],
                                                                         # initial_state_bw=initial_state["backward"])
            outputs_forward, outputs_backward = outputs_lstm
            output = tf.concat([outputs_forward, outputs_backward], axis=2, name='output_sequence')
        else:
            outputs_lstm, final_states = tf.nn.bidirectional_dynamic_rnn(GRU_cell["forward"], GRU_cell["backward"],
...
class EntityLSTM(object):
    ...
    def __init__(self, dataset, parameters):
        ...
        # Do not use CRF layer
            ...
            print('Using softmax for score calculation...')
            with tf.variable_scope("crf") as vs:
                self.transition_parameters = tf.get_variable(
                    "transitions",
                    shape=[dataset.number_of_classes + 2, dataset.number_of_classes + 2],
    ...
    def define_training_procedure(self, parameters):
        ...
        if parameters['gradient_clipping_value']:
            grads_and_vars = [(tf.clip_by_value(grad, -parameters['gradient_clipping_value'],
                                                parameters['gradient_clipping_value']), var) for grad, var in
